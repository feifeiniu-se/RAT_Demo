...
//
            ...
            if(methodBlock == null){
                mappings.keySet().stream().filter(p -> p.contains("DefaultRemoteRepositoriesService"))
...
//            assert left.get(0).getFilePath().equals(right.get(0).getFilePath());
            ...
            if(attriBlock == null){
                mappings.keySet().stream().filter(p -> p.contains("fr.jayasoft.ivy.namespace.MRIDTransformationRule.MridRuleMatcher:String")).forEach(System.out::println);
                System.out.println();
